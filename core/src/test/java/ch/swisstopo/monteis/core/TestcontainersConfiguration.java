...
class TestcontainersConfiguration {
    ...
    PostgreSQLContainer<?> timescaleDB(
            @Value("${ts.db.name}") String dbName,
            Network network) {
    ...
    DatabaseMigrations timeScaleMigrations(
        ...
        Flyway.configure()
                ...
                .placeholders(Map.of("fdw_read_user",fdwReadUser)) // let the fdw read user be controlled from outside
                .load()
                .migrate();
    ...
    DatabaseMigrations metaDataMigrations(
            ...
            @Qualifier("timeScaleMigrations") DatabaseMigrations tsDBMigrations, // enforce the Timescale migration to succeed first in order to have correct permissions for fdw. todo: check if proper!!
        ...
        Flyway.configure()
                ...
                .locations("filesystem:" + migrationRoot + "/db/meta/schema", "filesystem:" + migrationRoot + "/db/meta/seed")
                .load()
                .migrate();
        return new DatabaseMigrations();
    }
