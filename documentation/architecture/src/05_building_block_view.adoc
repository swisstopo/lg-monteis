...
* **Security & Auth Filter:** Verarbeitet die vom externen Keycloak/eIAM empfangenen JWTs. Über den Spring `@Authenticated` Principal werden die in den Claims enthaltenen Rollen (z. B. `read_exp_a`) geparst und das RBAC (Role-Based Access Control) auf API-Ebene strikt durchgesetzt.
