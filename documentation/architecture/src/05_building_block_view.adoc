...
* **Security & Auth Filter:** Der Authentifizierungs-Filter von Spring Security's OAuth2-Resource-Server verifiziert das Keycloak/eIAM-JWT und baut daraus über einen custom `Converter` in einem Schritt die vollständige `Authentication` auf: Rolle → `api:*`-Authorities, sowie Sub und Experiment-IDs als eigener `SecurityPrincipal`, abgelegt im Standard-`SecurityContextHolder`. Leseberechtigungen werden darüber transparent auf Datenbankebene per Row-Level-Security durchgesetzt, Schreibberechtigungen (RBAC) rollenbasiert auf Controller-Ebene (siehe Kap. 8 & 9).
