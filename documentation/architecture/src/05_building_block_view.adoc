...
* **Security & Auth Filter:** Zweistufige Filterkette: Ein Authentifizierungs-Filter verifiziert das Keycloak/eIAM-JWT und legt es im `SecurityContextHolder` ab; `SecurityContextFilter` bindet daraus Rolle und Experiment-IDs als `SecurityContext` in ein `ScopedValue`. Leseberechtigungen werden darüber transparent auf Datenbankebene per Row-Level-Security durchgesetzt, Schreibberechtigungen (RBAC) rollenbasiert auf Controller-Ebene (siehe Kap. 8 & 9).
