...
1. Der Nutzer öffnet die Web-App und wird vom Angular Frontend an Keycloak weitergeleitet (OIDC Authorization Code Flow) und authentifiziert sich (z. B. via AGOV). Keycloak stellt ein JWT aus, dessen Claims die Rollen (`realm_access.roles`, z. B. `ADMIN`/`USER`) sowie die Experiment-Zugehörigkeiten (`experiment_ids`-Claim) enthalten.
2. Das Frontend sendet dieses JWT bei jedem REST-Call im `Authorization: Bearer`-Header mit.
3. Ein Security-Filter im Spring Boot Backend (OAuth2 Resource Server) fängt den Request ab, verifiziert die Signatur des Tokens (via JWKS von Keycloak) und übergibt es an einen custom `Converter` (`JwtSecurityAuthenticationConverter`), der daraus in einem Schritt die vollständige `Authentication` baut: Rollen → `api:read`/`api:write`/`api:read-all`-Authorities (via `RealmRoleAuthoritiesConverter`), sowie Sub und Experiment-IDs als eigener `SecurityPrincipal`. Diese `Authentication` landet im Standard-`SecurityContextHolder`. Fehlt eine gültige Rolle (keine `api:read`/`api:read-all`-Authority), werden Experiment-IDs strikt auf "keine Rechte" verengt (*fail closed*), selbst wenn der `experiment_ids`-Claim befüllt wäre.
4. Bei jeder jOOQ-Datenbankverbindung schreibt ein custom `ConnectionProvider` (`RlsConnectionProvider`) die aktuelle `Authentication` transaktionslokal als Postgres-GUCs auf die Connection (`app.read_all` aus den Authorities, `app.user_experiment_ids` aus dem `SecurityPrincipal`).
