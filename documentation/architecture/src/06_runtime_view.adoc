...
1. Der Nutzer öffnet die Web-App und wird vom Angular Frontend an Keycloak weitergeleitet (OIDC Authorization Code Flow mit PKCE) und authentifiziert sich (delegiert an AGOV). Berechtigungen sind in Keycloak über Gruppenmitgliedschaft vergeben (z. B. eine Admin-Gruppe, eine Gruppe pro Experiment) statt direkt am Nutzer. Keycloak stellt ein JWT aus, dessen Claims die Berechtigungen sowie die Experiment-Zugehörigkeiten des Nutzers enthalten.
2. Das Frontend sendet dieses JWT bei jedem REST-Call im `Authorization: Bearer`-Header mit.
3. Ein Security-Filter im Spring Boot Backend (OAuth2 Resource Server) fängt den Request ab, verifiziert Signatur, Aussteller und Audience des Tokens (via JWKS von Keycloak) und übergibt es an einen custom `Converter`, der daraus die vollständige `Authentication` baut und im Standard-`SecurityContextHolder` ablegt. Fehlt eine gültige Leseberechtigung, werden Experiment-IDs strikt auf "keine Rechte" verengt (*fail closed*), selbst wenn der Claim befüllt wäre.
4. Bei jeder jOOQ-Datenbankverbindung schreibt ein custom `ConnectionProvider` (`RlsConnectionProvider`) die aktuelle `Authentication` transaktionslokal als Postgres-GUCs auf die Connection.
5. Native **Row-Level-Security-Policies** auf `sensors`/`experiments` sowie eine `security_barrier`-View für die (per Foreign Data Wrapper eingebundenen) Sensormesswerte werten diese GUCs serverseitig aus und filtern Zeilen automatisch — der jOOQ-Code selbst enthält keine manuelle Berechtigungs-`WHERE`-Klausel. Admins sehen alles; reguläre Nutzer nur Daten aus Experimenten, deren Gruppe sie angehören.
6. Schreibende Requests (`POST`/`PUT`/`PATCH`/`DELETE`) werden unabhängig davon zentral im Filterchain (`SecurityConfig`) geprüft und erfordern eine Write-Berechtigung. Das Frontend fragt diese zusätzlich über `GET /api/me` ab, um schreibende UI-Elemente rein kosmetisch auszublenden — die eigentliche Durchsetzung bleibt serverseitig im Filterchain.

Die Experiment-Zugehörigkeit wird aktuell bewusst als JWT-Claim geführt, gespeist aus der Gruppenmitgliedschaft des Nutzers. Das ist einfach zu betreiben, solange die Zugehörigkeitsmengen klein bleiben — in der Praxis meist ein einzelnes Experiment pro Nutzer. Sollte das nicht mehr skalieren (viele Experimente pro Nutzer, häufig wechselnde Zuordnungen, oder schlicht Token-Grössenlimits), ist die vorgesehene Ausweichlösung eine eigene `experiment_membership`-Tabelle (Schlüssel: `sub`), die der `MonteisJwtAuthenticationConverter` pro Request abfragt, statt die Zugehörigkeit aus dem Token zu lesen.
