...
3. Ein Security-Filter im Spring Boot Backend fängt den Request ab, verifiziert die Signatur des Tokens (via JWKS von Keycloak) und legt es als Principal in den `SecurityContextHolder`.
4. Ein `SecurityContextFilter` liest Rollen und Experiment-IDs aus diesem JWT und bindet sie für die Dauer des Requests als `SecurityContext` (Access Level + Experiment-IDs) in ein Java `ScopedValue`. Fehlt eine gültige Rolle, wird strikt auf "keine Rechte" verengt (*fail closed*).
5. Bei jeder jOOQ-Datenbankverbindung schreibt ein custom `ConnectionProvider` (`RlsConnectionProvider`) den aktuellen `SecurityContext` transaktionslokal als Postgres-GUCs auf die Connection.
6. Native **Row-Level-Security-Policies** auf `sensors`/`experiments` sowie eine `security_barrier`-View für die (per Foreign Data Wrapper eingebundenen) Sensormesswerte werten diese GUCs serverseitig aus und filtern Zeilen automatisch — der jOOQ-Code selbst enthält keine manuelle Berechtigungs-`WHERE`-Klausel. Admins sehen alles; reguläre Nutzer nur Daten aus Experimenten, denen sie zugeordnet sind.
