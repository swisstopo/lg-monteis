...
Die Architektur verwendet das **Adapter-Pattern** (`AbstractNormalizationAdapter`), um neue Datenquellen nahtlos als "Plug-and-Play"-Komponenten zu integrieren. Die Kern-Pipeline (alles ab dem `internal-normalized` Topic) bleibt von der Herkunft der Daten vollständig entkoppelt.
...
=== 5.3. Normalization Adapter implementieren
Dies ist die eigentliche Übersetzungslogik innerhalb der Java-Applikation.

* Erstellen eines neuen Spring `@Service`, der die abstrakte Klasse `AbstractNormalizationAdapter` erweitert.
* Hinzufügen eines `@KafkaListener`, der auf das neu erstellte Topic lauscht.
* Innerhalb des Listeners die Rohdaten in das Standardformat (`NormalizedSensorData`) umwandeln.
* **Wichtig - Guarded Acknowledgment:** Um Datenverlust zu vermeiden, muss das Acknowledgment-Pattern exakt repliziert werden:
1. Aufruf von `publishNormalizedData(sensorId, canonicalPayload)`.
2. Das resultierende `CompletableFuture` erfassen und auf Bestätigung warten (`CompletableFuture.allOf(...).join()`).
3. **Erst nach erfolgreicher Bestätigung** darf der Offset des Raw-Topics via `ack.acknowledge()` committet werden.
...
Der Java-basierte `AbstractNormalizationAdapter` bleibt somit ausschliesslich für Datenquellen reserviert, die komplexe Restrukturierungen oder bedingte Logik erfordern.
