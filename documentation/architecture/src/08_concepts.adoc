...
* **JaVers-Annotationen:** Das DTO _muss_ die `@Id`-Annotation (aus `org.javers.core.metamodel.annotation.Id`) auf dem Primärschlüssel tragen, damit JaVers den Datensatz als eigenständige Entity trackt. Die Nutzung von `@TypeName("WunschName")` wird dringend empfohlen, um den Audit-Log von Java-Refactorings zu entkoppeln.
