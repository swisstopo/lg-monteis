...
public class AlertPublisher {
    ...
    public void handleBoundBreach(SensorBoundBreachedEvent event) {
        log.info("Preparing to send alert for sensor {}: value {} exceeded {} bound of {}",
                event.sensorId(), event.standardizedValue(), event.boundType(), event.limitViolated());
        ...
        kafkaTemplate.send(alertTopic, event.sensorId(), event)
