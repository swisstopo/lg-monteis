...
class NormalizedSensorDataPublisherTest {
  ...
  @Mock private KafkaTemplate<String, NormalizedSensorData> kafkaTemplate;

  @Test
  void should_delegate_to_kafka_template() {
    // given
    String topic = "test-topic";
    NormalizedSensorDataPublisher publisher =
        new NormalizedSensorDataPublisher(kafkaTemplate, topic);

    String sensorId = "deviceA";
    NormalizedSensorData payload = new NormalizedSensorData(sensorId, "2026-06-23T09:00:00Z", 15.5);
    CompletableFuture<SendResult<String, NormalizedSensorData>> expectedFuture =
        new CompletableFuture<>();

    given(kafkaTemplate.send(topic, sensorId, payload)).willReturn(expectedFuture);

    // when
    CompletableFuture<SendResult<String, NormalizedSensorData>> actualFuture =
        publisher.publish(sensorId, payload);
