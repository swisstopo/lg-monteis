...
class TransformationOrchestratorTest {
    ...
    @Mock
    private SIStandardizer siStandardizer;
    ...
    void shouldSuccessfullyParseEpochTimestampAndOrchestrate() {
        ...
        given(boundsValidator.evaluateBounds("deviceA", standardizedValue, defaultConfig)).willReturn(BoundStatus.OK);

        // when
        SensorReadingRecord result = orchestrator.transform("deviceA", rawValue, rawTimestamp, defaultConfig);
        ...
        OffsetDateTime expectedTimestamp = OffsetDateTime.ofInstant(Instant.ofEpochMilli(epochMilli), ZoneOffset.UTC);
    ...
    void shouldMapTooHighStatusToCorrectDatabaseEnum() {
        ...
        given(siStandardizer.standardizeToSI(rawValue, defaultConfig)).willReturn(standardizedValue);
        given(boundsValidator.evaluateBounds("deviceA", standardizedValue, defaultConfig)).willReturn(BoundStatus.TOO_HIGH);

        // when
        SensorReadingRecord result = orchestrator.transform("deviceA", rawValue, defaultTimestamp, defaultConfig);
    ...
    void shouldMapTooLowStatusToCorrectDatabaseEnum() {
        ...
        given(siStandardizer.standardizeToSI(rawValue, defaultConfig)).willReturn(standardizedValue);
        given(boundsValidator.evaluateBounds("deviceA", standardizedValue, defaultConfig)).willReturn(BoundStatus.TOO_LOW);
    ...
    void shouldWrapMathExceptionsIntoTransformationException() {
        ...
        // (Assuming your TransformationException constructor accepts the rawValue and exposes it via getFailedPayload())
