...
class ReprocessChunkServiceTest {
    ...
    @BeforeEach
    void setUp() {
        chunkService = new ReprocessChunkService(
    ...
    void should_process_and_bulk_update_valid_records() throws TransformationException {
        ...
        SensorReadingRecord oldRecord = new SensorReadingRecord(
                timestamp, "deviceA", 10.0, 15.0, (short) 1, RangeCategory.correct
        );

        given(sensorReadingRepository.fetchOldSensorData(config, testChunkSize))
                .willReturn(List.of(oldRecord));

        SensorReadingRecord transformedRecord = new SensorReadingRecord(
                timestamp, "deviceA", 10.0, 20.0, (short) 2, RangeCategory.correct
        );
    ...
    void should_handle_poison_pills_by_bumping_version_and_setting_null_norm_value() throws TransformationException {
        ...
        SensorReadingRecord poisonRecord = new SensorReadingRecord(
        ...
        given(transformationOrchestrator.transform("deviceB", -999.0, timestamp, config))
                .willThrow(ex);
