  ...
  return next(req).pipe(
    catchError((error: unknown) => {
      if (error instanceof HttpErrorResponse) {
        ...
        if (error.status === 401) {
          toastService.error(translateService.instant('error.auth.sessionExpired'), undefined, {
            label: translateService.instant('error.auth.logInAgain'),
            onClick: () => oauthService.initLoginFlow(), // re-login
          });
        } else if (error.status === 403) {
          toastService.error(translateService.instant('error.auth.forbidden'));
        } else {
          ...
          if (globalErrors.length > 0) {
            globalErrors.forEach((err) =>
              toastService.error(
                translateService.instant(err.messageKey ?? 'error.system.internal'),
              ),
            );
